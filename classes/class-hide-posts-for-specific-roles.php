...
class Hide_Posts_For_Specific_Roles {
   ...
   <div class="wrap">
	   ...
	   <ul class="subsubsub hpfsr-subsubsub">
		  	<li class=""><?php _e( 'Filter by: ', 'hpfsr' );?></a></li>
		  	<li class="all"><a class="<?php echo ( $_GET['post_status'] == 'any' ) ? 'active' : '' ?>" href="tools.php?page=hpfsr-settings&post_status=any"><?php _e( 'All', 'hpfsr' );?></a></li>
		  	<?php foreach ($usable_post_status as $key_status => $label_status) {?>
			<li class="<?php echo $key_status ?>">| <a class="<?php echo ( $_GET['post_status'] == $key_status ) ? 'active' : '' ?>" href="tools.php?page=hpfsr-settings&post_status=<?php echo $key_status ?>"><?php echo $label_status ?></a></li>
		  	<?php } ?>
		</ul>
	   	<form method="post" action="options.php" class="hpfsr-form"> 
		  	<legend><?php printf( __( 'Choose posts with status <strong> %1$s </strong> that <em class="red-text">NOT BE SHOWN</em> in the list for each role.', 'hpfsr' ), self::get_usable_post_status_label( $_GET['post_status'] ) ) ?></legend>
		  	<?php submit_button(); ?>
		   	<?php
		   		settings_errors();
			  	...
			  	<div id="postbox-container" class="postbox-container">
					<div class="meta-box-sortables ui-sortable" id="normal-sortables">
				  	<?php 
								...
								foreach ( $posts as $post ){ ?>
									<li class="hpfsr-post-name">
										...
										<ul class="hpfsr-select-roles-option">
											...
											<li><label><input type="checkbox" name="select-all-less-administrator" data-selectalllapostid="<?php echo $post->ID?>" id="select-all-less-administrator-<?php echo $post->ID?>" value="on"><?php echo __( 'Select all <strong>less administrator</strong>', 'hpfsr' )?></label></li>
										</ul>
				  		...
				  		if( $p_count == 0 ){
				  			printf( __( '<p class="hpfsr-alert info">No posts found with status <strong>%1$s</strong>.</p>', 'hpfsr' ), self::get_usable_post_status_label( $_GET['post_status'] ) );
				  			echo '<style>p.submit, .hpfsr-form legend{display:none}</style>';
				  		}
				  		?>
					</div>
			  	</div>
   ...
   */
	...
	* @return string usable post status label
	* @author Marcelo Torres
	* @since 1.1.0
	*
	*/
   	private static function get_usable_post_status_label( $status ) {

		$get_post_stati = get_post_stati($args = array(), $output = 'objects', $operator = 'and');

		return $get_post_stati[$status]->label;
   	}
